...
class EmotionResNet(nn.Module):
    ...
    def __init__(self, num_classes=8):
        ...
        # Initial convolution (8 input channels for 8 features)
        self.conv1 = nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        # x should already be 8-channel (batch, 8, height, width)
        if x.dim() == 3:
...
class EmotionDenseNet(nn.Module):
    ...
    def __init__(self, num_classes=8, growth_rate=32, block_config=(6, 12, 24, 16), 
        ...
        self.features = nn.Sequential(
            nn.Conv2d(8, num_init_features, kernel_size=7, stride=2, padding=3, bias=False),
